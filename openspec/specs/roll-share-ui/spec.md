...
- Proposal element "Dice panel as flex sibling replacing the document.body portal" → Requirements: ADDED Dice panel renders as an in-flow flex sibling to the left of the chat dock; REMOVED Floating dice pop-out renders outside the chat dock's DOM subtree
- Proposal element "Auto-scroll feed to the user's own newly-posted roll" → Requirements: ADDED Feed auto-scrolls to a roll the current user just posted
