...
Persist each authenticated user's non-default UI preferences (dice-fab and chat-dock settings) server-side so they follow the user across sessions and devices, while keeping an offline-first local mirror for instant first paint. Implementation: `lib/preferences/schema.ts`, `lib/preferences/usePreferences.tsx`, `lib/storage/userPreferencesRepo.ts`, `app/api/me/preferences/route.ts`. Design rationale: `openspec/changes/archive/2026-09-01-add-user-preference-persistence/design.md`.

## Requirements

### Requirement: Preferences persist across sessions and devices
...
### Requirement: Changing a preference persists it
...
### Requirement: Invalid preference updates are rejected
...
### Requirement: Existing local preferences are adopted on first login
...
### Requirement: Existing preference hooks keep their contract
...
### Requirement: Preferences sync across tabs
...
### Requirement: Anonymous users use local-only preferences
...
### Requirement: Preference persistence degrades gracefully
...
### Requirement: User model carries optional preferences
...
The system SHALL load preferences with at most one additional HTTP request beyond the existing bootstrap, running concurrently with other bootstrap fetches, and SHALL render preference-bound UI from the local mirror without waiting on the network.
...
The system SHALL scope every preference read and write to the authenticated session's user id, reject unauthenticated requests, and reject payloads that do not match the preference schema.
...
The system SHALL re-send a failed preference delta on the next hydration or change so the server converges to the local value, and SHALL resolve corrupt or stale-version stored data to current defaults merged with only the valid known deltas without raising an error.
...
The system SHALL route `getUserPreferences` and `updateUserPreferences` through the shared storage-operation telemetry seam using the same event shape as other storage operations.
