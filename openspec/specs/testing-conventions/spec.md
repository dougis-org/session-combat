...
Conventions for naming test-file helpers in `tests/unit/components/CampaignChat/` so local, file-scoped helpers never collide with `helpers.tsx`'s exported helpers. See [`design.md`](../../changes/archive/2026-09-18-fix-resize-test-opendock-shadow/design.md) and [`tasks.md`](../../changes/archive/2026-09-18-fix-resize-test-opendock-shadow/tasks.md) for the change that introduced these requirements.

## Requirements

### Requirement: No local test helper shadows a `helpers.tsx` export name
...
### Requirement: Existing resize-test behavior is preserved across the rename
...
### Requirement: `helpers.tsx` documents the naming-collision risk for local test helpers
