## Purpose
Define the machine-readable inventory and fixed taxonomy documenting the current (pre-refactor) error-handling behavior of every method on the `storage` object exported by `lib/storage.ts`, plus the characterization tests that pin that behavior ahead of the storage-layer decomposition (#499).
