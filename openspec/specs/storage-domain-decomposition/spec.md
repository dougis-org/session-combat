...
Define the per-domain decomposition of the monster-template, campaign-template,
campaign, and campaign-membership storage methods out of the `lib/storage.ts`
facade into dedicated repo modules, and the error-handling contract (real
failures reject with `StorageError`; empty/not-found stay non-throwing) that the
migrated methods follow. Established by
[#503](../../changes/archive/2026-09-02-refactor-storage-issue-503/design.md).

## Requirements

### Requirement: Campaign and template domain methods live in per-domain repos
...
### Requirement: Public storage facade shape is preserved
...
### Requirement: Storage failures in the migrated cluster surface as StorageError
...
### Requirement: Empty and not-found results do not throw
...
### Requirement: Campaign access checks do not mask storage failures as not-found
...
`Date.now()` bracketing, and total unit/integration suite runtime SHALL NOT
...
See also functional scenarios "Database failure during a previously-swallowing
read" and "Database outage during an access check".
