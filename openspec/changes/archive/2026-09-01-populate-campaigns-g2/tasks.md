...
- [x] 5.1 Spawn sub-agent to run `openspec-review-code` skill on the changed files (`lib/data/customMonsters.ts`, `lib/scripts/seedCampaignTemplates.ts`, `tests/unit/lib/scripts/seedCampaignTemplates.test.ts`).
- [x] 5.2 Apply all clearly-correct findings to the code (no stopping, no user prompt).
- [x] 5.3 Re-run `npm run test:unit` after applying fixes; confirm all tests pass.

## 6. Validation

- [x] 6.1 `npm run typecheck` exits clean.
- [x] 6.2 `npm run test:unit` exits clean — all 5 new G2 tests pass.
- [x] 6.3 `npx eslint lib/data/customMonsters.ts lib/scripts/seedCampaignTemplates.ts tests/unit/lib/scripts/seedCampaignTemplates.test.ts` exits clean.

## 7. PR and Merge

- [x] 7.1 Open a PR from `feature/populate-campaigns-g2` to `main` with title `feat(campaigns): populate G2 encounters (WDH, SKT, OotA, DIP, PaBtSO)` and a body modeled on PR #647. (PR #661)
- [x] 7.2 Enable auto-merge on the PR.
- [x] 7.3 Wait 3 minutes for CI to start.
- [x] 7.4 Spawn sub-agent to run `pr-review-toolkit:review-pr` on the PR.
- [x] 7.5 Address all findings from the review (commit, push, re-run) until zero findings remain.
- [x] 7.6 Monitor PR for new comments and CI failures autonomously; iterate until merged.
- [x] 7.7 After merge, verify changes appear on `main`.

## 8. Post-Merge

- [x] 8.1 Sync approved spec deltas from `openspec/changes/populate-campaigns-g2/specs/` back to `openspec/specs/`.
...
- [x] 9.1 Update the `## Status` table in `docs/campaign-encounter-rollout.md` to mark WDH, SKT, OotA, DIP, PaBtSO as ✅ merged with the new PR number.
