...
- [x] Archive: move `openspec/changes/password-reset-api/` → `openspec/changes/archive/YYYY-MM-DD-password-reset-api/` in a single commit
- [x] Confirm archive location exists and original directory is gone
- [x] Commit and push the archive to `main`
- [x] Prune merged branch: `git fetch --prune` and `git branch -d feat/password-reset-api`
