...
#### Scenario: Ingestion phase no longer returns partial success
...
The system SHALL limit the `/monsters/import` page to the administrative open5e sync function and SHALL NOT provide a JSON-file upload form on that page, since user-facing JSON import is handled by the import modal on `/monsters`.
...
### Requirement: REMOVED Partial-success monster upload

Reason for removal: The bulk upload endpoint previously imported whatever rows it could and returned HTTP 207 with a per-row error list, allowing a partially-applied import. Issue #626 requires that a failed import reverts entirely and the user never sees a half import. Replaced by the "Atomic-ish ingestion with revert on failure" and "Monster JSON upload endpoint" requirements above.

## Traceability

- Proposal element "Import Monster(s) button for all authenticated users" -> Requirement: Import entry point on the monster library
- Proposal element "Downloadable JSON structure document" -> Requirement: Downloadable import structure document
- Proposal element "Validate whole file, then preview count + names" -> Requirements: Whole-file validation before preview, Import preview, Monster JSON upload endpoint
- Proposal element "Admin whole-batch Global vs Personal, enforced server-side" -> Requirement: Import scope selection
- Proposal element "Dedupe skip + skipped list shown" -> Requirement: Duplicate handling on import
- Proposal element "No half import / revert on error" -> Requirement: Atomic-ish ingestion with revert on failure
- Proposal element "Single schema source (Zod)" -> Requirements: Whole-file validation before preview, Downloadable import structure document
- Proposal element "Remove duplicate file-upload UI" -> Requirement: Monster import page scope; REMOVED Partial-success monster upload
- Design Decision 1 (Zod single source of truth) -> Requirements: Whole-file validation before preview, Downloadable import structure document
- Design Decision 2 (required-field set) -> Requirement: Whole-file validation before preview
- Design Decision 3 (two routes, client re-sends file) -> Requirements: Monster JSON upload endpoint, Import preview
- Design Decision 4 (validate-all -> dedupe -> bulk insert -> compensating delete) -> Requirements: Duplicate handling on import, Atomic-ish ingestion with revert on failure
- Design Decision 5 (structure document) -> Requirement: Downloadable import structure document
- Design Decision 6 (modal + page integration) -> Requirements: Import entry point on the monster library, Import preview, Import error reporting in the modal
- Design Decision 7 (keep open5e page, drop upload panel) -> Requirement: Monster import page scope
- Requirement: Import entry point on the monster library -> Tasks: UI/modal tasks in `tasks.md`
- Requirement: Downloadable import structure document -> Tasks: schema-descriptor + schema route tasks
- Requirement: Whole-file validation before preview -> Tasks: Zod schema + validate route tasks
- Requirement: Import preview -> Tasks: modal preview + validate route tasks
- Requirement: Import scope selection -> Tasks: ingest route admin-check + transform scope tasks
- Requirement: Duplicate handling on import -> Tasks: repo dedupe lookup + ingest dedupe tasks
- Requirement: Atomic-ish ingestion with revert on failure -> Tasks: bulk insert + compensating delete repo/route tasks
- Requirement: Import error reporting in the modal -> Tasks: modal error-region task
- Requirement: Monster JSON upload endpoint -> Tasks: route split tasks
- Requirement: Monster import page scope -> Tasks: import page cleanup task

## Non-Functional Acceptance Criteria

> NFAC scenarios below express non-functional properties not already covered by the functional scenarios above.
...
- **And** the server also rejects an over-cap body if one is received

### Requirement: Security

- Global-scope access control is fully specified by the functional scenarios "Admin imports to the Global library" and "Forged Global request from a non-admin is denied" under *Import scope selection*. See those functional scenarios.
...
- **Then** those ids are logged as orphaned monster ids on the server
