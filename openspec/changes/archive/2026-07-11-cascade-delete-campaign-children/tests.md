...
All test cases live in `tests/unit/storage/campaigns.test.ts` (extending the existing `describe("storage.deleteCampaign", ...)` block) and integration test coverage in `tests/integration/campaigns.integration.test.ts`. The unit tests mock the underlying MongoDB collections and assert `deleteMany`/`deleteOne` are invoked with the correct filters and call order — they do not seed or verify removal of real documents; that end-to-end behavior is covered by the integration tests below.

- [x] **Test 1** (Task: implement cascade delete for `Party`; Scenario: "Campaign with multiple parties is deleted"): Mock the `parties` collection's `deleteMany`. Call `storage.deleteCampaign`. Assert `deleteMany` is called with `{ campaignId: id }` (a `campaignId`-only filter naturally excludes unrelated campaigns' parties).
- [x] **Test 2** (Task: implement cascade delete for `Party`; Scenario: "Campaign with no parties is deleted"): Call `storage.deleteCampaign` for a campaign with the `parties` collection mocked to resolve `{ deletedCount: 0 }`. Assert the call resolves without throwing.
- [x] **Test 3** (Task: implement cascade delete for `CampaignMember`; Scenario: "Campaign with multiple members across different users is deleted"): Mock the `campaignMembers` collection's `deleteMany`. Call `storage.deleteCampaign` as the DM. Assert `deleteMany` is called with `{ campaignId: id }` (no `userId` filter), so members across all `userId`s are covered, including the player's.
- [x] **Test 4** (Task: implement cascade delete for campaign child collections; Scenario: "Campaign with session history, rolls, character shares, saved content, and messages is deleted"): Mock the `sessionLogs`, `campaignRolls`, `campaignCharacterShares`, `savedContent`, and `campaignMessages` collections' `deleteMany`. Call `storage.deleteCampaign`. Assert each is called with the correct `{ campaignId: id }` filter.
