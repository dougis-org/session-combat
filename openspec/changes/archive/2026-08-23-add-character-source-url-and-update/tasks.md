...
- [x] Add `externalSync?: { provider: 'dndbeyond'; url: string; lastSyncedAt?: Date }` to `Character` interface in `lib/types.ts`
- [x] Modify `POST /api/characters/import` route to save `externalSync` data into the database record when a URL is provided
- [x] Create a "Sync from D&D Beyond" button and warning modal UI component for the Character view
- [x] Connect the modal's Confirm action to trigger `POST /api/characters/import` with `url` and `overwrite: true`
...
- Reviewer(s): Current User
- Required approvals: 1 (via pr-review-toolkit)
