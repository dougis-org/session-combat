...
- [x] 6.5 Commit all changes, push the branch, and open a PR against the default branch with a `Closes #512` line in the body (per `proposal.md`'s GitHub issue reference), then enable auto-merge. PR: https://github.com/dougis-org/session-combat/pull/513 (auto-merge enabled, squash).
