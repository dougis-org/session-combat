...
- [x] Run security/code quality checks required by project standards (Codacy/Verity gate) (CI Codacy + ci-gate all passed)
...
- [x] Open PR from `fix-resize-test-opendock-shadow` to `main`. PR body **must include `Closes #568`**. (PR #765)
- [x] **Issue lifecycle: mark in-review** — run `gh issue edit 568 --add-label "in-review" --remove-label "in-progress"`. Move the project item to the status column semantically matching "In Review" via `gh project item-edit` (same discovery pattern as above; warn and skip if not found).
...
- [x] **Enable auto-merge only after the review gate passes (zero findings):** `gh pr merge <PR-URL> --auto --squash` (per repo's squash-only branch ruleset; NEVER use `--admin` to force the merge)
- [x] **Iterate until merged** (PR #765 merged as e8051864) — repeat the following priority loop continuously until `gh pr view <PR-URL> --json state` returns `MERGED`; if it returns `CLOSED` exit and notify the user — **never wait for a human to report the merge; never force-merge**:
...
- [x] Update repository documentation impacted by the change (none expected — test-only change; confirmed none needed)
- [x] Sync approved spec deltas into `openspec/specs/`: copy `specs/testing-conventions/spec.md` to `openspec/specs/testing-conventions/spec.md` (create the capability if it doesn't already exist), updating any relative links to point to `../../changes/archive/YYYY-MM-DD-fix-resize-test-opendock-shadow/design.md` and `.../tasks.md`
- [x] Archive the change: move `openspec/changes/fix-resize-test-opendock-shadow/` to `openspec/changes/archive/YYYY-MM-DD-fix-resize-test-opendock-shadow/`, staging both the new location and the deletion of the old location in a single commit
- [x] Confirm `openspec/changes/archive/YYYY-MM-DD-fix-resize-test-opendock-shadow/` exists and `openspec/changes/fix-resize-test-opendock-shadow/` is gone
- [x] **Create a doc branch** for the archive and spec updates: `git checkout -b doc/archive-YYYY-MM-DD-fix-resize-test-opendock-shadow` then `git push -u origin doc/archive-YYYY-MM-DD-fix-resize-test-opendock-shadow`
- [x] Open a PR from `doc/archive-YYYY-MM-DD-fix-resize-test-opendock-shadow` to `main` with title `docs: archive fix-resize-test-opendock-shadow (YYYY-MM-DD)` — do NOT push directly to `main`
...
- [x] Prune merged local branches: `git fetch --prune` and `git branch -D fix-resize-test-opendock-shadow doc/archive-YYYY-MM-DD-fix-resize-test-opendock-shadow`
- [x] Remove the change's dedicated worktree: `git worktree remove .worktrees/fix-resize-test-opendock-shadow` (from the primary checkout)
