...
- [x] **Step 2 — Create and publish working branch:** `git checkout -b feature/populate-campaigns-g3` then immediately `git push -u origin feature/populate-campaigns-g3`
...
- [x] Open PR from working branch to `main`. **The PR body MUST include `Closes #581`** (unconditionally, not as an optional conditional).
- [x] **Issue lifecycle: mark in-review** _(skip if change is not issue-driven)_: run `gh issue edit #581 --add-label "in-review" --remove-label "in-progress"`. Then move the project item to the status column semantically matching "In Review" via `gh project item-edit` (same project/field/option discovery as the in-progress lifecycle step above; warn and skip if not found).
...
- [x] Sync approved spec deltas into `openspec/specs/` (global spec). After copying each `spec.md` to `openspec/specs/<cap>/spec.md`, update all relative links that pointed into the change directory so they resolve from the archive location — replace `../../design.md` with `../../changes/archive/YYYY-MM-DD-populate-campaigns-g3/design.md`, and similarly for `../../tasks.md` and any other relative paths into the change directory.
- [x] Archive the change: move `openspec/changes/populate-campaigns-g3/` to `openspec/changes/archive/YYYY-MM-DD-populate-campaigns-g3/` **and stage both the new location and the deletion of the old location in a single commit** — do not commit the copy and delete separately
- [x] Confirm `openspec/changes/archive/YYYY-MM-DD-populate-campaigns-g3/` exists and `openspec/changes/populate-campaigns-g3/` is gone
- [x] **Create a doc branch** for the archive and spec updates: `git checkout -b doc/archive-YYYY-MM-DD-populate-campaigns-g3` then `git push -u origin doc/archive-YYYY-MM-DD-populate-campaigns-g3`
- [x] Open a PR from `doc/archive-YYYY-MM-DD-populate-campaigns-g3` to `main` with title `docs: archive populate-campaigns-g3 (YYYY-MM-DD)` — **do NOT push directly to `main`**
...
- [x] Prune merged local branches: `git fetch --prune` and `git branch -D feature/populate-campaigns-g3 doc/archive-YYYY-MM-DD-populate-campaigns-g3`

Required cleanup after archive: `git fetch --prune` and `git branch -D feature/populate-campaigns-g3 doc/archive-YYYY-MM-DD-populate-campaigns-g3`
