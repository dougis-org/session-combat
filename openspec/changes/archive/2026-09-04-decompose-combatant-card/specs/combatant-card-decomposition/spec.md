...
### Requirement: ADDED `CombatantCard` is a thin composition layer
...
### Requirement: ADDED HP controls extracted to `HpControls`
...
### Requirement: ADDED Condition controls extracted to `ConditionControls`
...
### Requirement: ADDED Card header extracted to `CombatantCardHeader`
...
### Requirement: MODIFIED `CombatantCard` no longer imports unused action panels
...
### Requirement: REMOVED `window.prompt`-based condition entry for the current combatant

Reason for removal: The two `window.prompt` calls in `addCondition` are replaced
by the `ConditionFormModal` component. Condition data, validation limits
(name <= 100 chars, duration `[1, 10_000]`), and the resulting `onUpdate`
payload are unchanged.

## Traceability

- Proposal element "Reduce CombatantCard.tsx to a <300-line composition layer" -> Requirement: ADDED `CombatantCard` is a thin composition layer
- Proposal element "Extract HpControls / ConditionControls / TargetingPanel / CombatantCardHeader" -> Requirements: ADDED HP controls extracted to `HpControls`, ADDED Condition controls extracted to `ConditionControls`, ADDED Card header extracted to `CombatantCardHeader` (TargetingPanel: see `target-action-modal` delta)
- Proposal element "Replace window.prompt() self condition-add with a real modal" -> Requirement: ADDED Condition controls extracted to `ConditionControls`; Requirement: REMOVED `window.prompt`-based condition entry for the current combatant
- Proposal element "Legendary/lair wiring out of scope" -> Requirement: MODIFIED `CombatantCard` no longer imports unused action panels
- Design Decision 4 -> Requirements: ADDED `CombatantCard` is a thin composition layer, ADDED HP controls extracted to `HpControls`, ADDED Card header extracted to `CombatantCardHeader`
- Design Decision 5 -> Requirement: ADDED Condition controls extracted to `ConditionControls`; Requirement: REMOVED `window.prompt`-based condition entry
- Design Decision 7 -> Requirement: MODIFIED `CombatantCard` no longer imports unused action panels
- Requirement: ADDED `CombatantCard` is a thin composition layer -> Tasks: "Create `lib/components/combatant-card/` and move sub-components", "Rewrite `CombatantCard.tsx` as composition layer", "Verify line count < 300"
- Requirement: ADDED HP controls extracted to `HpControls` -> Task: "Extract `HpControls.tsx`"
- Requirement: ADDED Condition controls extracted to `ConditionControls` -> Tasks: "Extract `ConditionControls.tsx`", "Create `ConditionFormModal.tsx`", "Add modal + controls tests"
- Requirement: ADDED Card header extracted to `CombatantCardHeader` -> Task: "Extract `CombatantCardHeader.tsx`"
- Requirement: MODIFIED `CombatantCard` no longer imports unused action panels -> Task: "Remove dead `LegendaryActionsPanel` / `LairActionsSlot` imports"
- Requirement: REMOVED `window.prompt`-based condition entry -> Task: "Delete `window.prompt` calls from condition-add flow"
