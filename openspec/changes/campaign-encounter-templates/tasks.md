...
- [x] Update `lib/scripts/seedCampaignTemplates.ts`: Add at least one default encounter to the `Dragon of Icespire Peak` template.
- [x] Update `app/api/campaigns/global/[id]/copy/route.ts`:
  - [x] Import `randomUUID` from `crypto` (already there).
  - [x] Import `storage` methods if missing.
  - [x] After copying the `Campaign` and before returning, iterate through `template.encounters`.
  - [x] For each `EncounterTemplate`, construct a full `Encounter` object: generate a new `id`, set `userId` to `auth.userId`, copy name/description/monsters, set `createdAt`/`updatedAt` to `new Date()`.
  - [x] Save each `Encounter` via `storage.saveEncounter`.
  - [x] Push the generated IDs into a `const encounterIds = []` array.
  - [x] Add `encounterIds` to the `campaign` object (or update the campaign document after insertion).
  - [x] Wrap this in the existing `try/catch` block that cleans up the campaign if member addition fails, so if encounter creation fails, the campaign is also rolled back.
