...
- [x] **Step 2 — Create and publish working branch:** `git checkout -b session-log-inline-edit` then immediately `git push -u origin session-log-inline-edit`
...
- [x] **Issue lifecycle: mark in-progress**: run `gh issue edit 723 --add-label "in-progress"`. Then discover the GitHub Project linked to the repo (`gh project list --owner dougis-org --format json`), resolve the status field option semantically matching "In Progress" (`gh project field-list <project-number> --owner dougis-org --format json`), and move the project item via `gh project item-edit`. If no project item is found, log a warning and continue. If the `gh` token lacks the `project` scope, surface a message instructing the user to run `gh auth refresh -s project` and skip the project-item update (issue label update still proceeds).
- [x] Update `app/campaigns/[id]/sessions/page.tsx` top-level render condition for `SessionForm` to only show when `showForm && !editingLog`
- [x] Update `app/campaigns/[id]/sessions/page.tsx` `logs.map` to render `SessionForm` inline if `editingLog?.id === log.id`
...
- [ ] Sync approved spec deltas into `openspec/specs/` (global spec). After copying each `spec.md` to `openspec/specs/<cap>/spec.md`, update all relative links that pointed into the change directory so they resolve from the archive location — replace `../../design.md` with `../../changes/archive/YYYY-MM-DD-session-log-inline-edit/design.md`, and similarly for `../../tasks.md` and any other relative paths into the change directory.
- [ ] Archive the change: move `openspec/changes/session-log-inline-edit/` to `openspec/changes/archive/YYYY-MM-DD-session-log-inline-edit/` **and stage both the new location and the deletion of the old location in a single commit** — do not commit the copy and delete separately
- [ ] Confirm `openspec/changes/archive/YYYY-MM-DD-session-log-inline-edit/` exists and `openspec/changes/session-log-inline-edit/` is gone
- [ ] **Create a doc branch** for the archive and spec updates: `git checkout -b doc/archive-YYYY-MM-DD-session-log-inline-edit` then `git push -u origin doc/archive-YYYY-MM-DD-session-log-inline-edit`
- [ ] Open a PR from `doc/archive-YYYY-MM-DD-session-log-inline-edit` to `main` with title `docs: archive session-log-inline-edit (YYYY-MM-DD)` — **do NOT push directly to `main`**
- [ ] **IMMEDIATELY** enable auto-merge on the doc PR: `gh pr merge <DOC-PR-URL> --auto --merge` (NEVER use `--admin` to force the merge)
- [ ] Monitor the doc PR until it merges (same loop as the implementation PR — address comments and CI failures, push to the same doc branch, repeat)
- [ ] Prune merged local branches: `git fetch --prune` and `git branch -D session-log-inline-edit doc/archive-YYYY-MM-DD-session-log-inline-edit`

Required cleanup after archive: `git fetch --prune` and `git branch -D session-log-inline-edit doc/archive-YYYY-MM-DD-session-log-inline-edit`
