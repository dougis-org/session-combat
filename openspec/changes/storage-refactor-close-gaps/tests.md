...
- [x] `savedContentRepo.create` resolves to the same `SavedContent` shape on
      success and rejects with `StorageError` (no swallow, no behavior
      change from pre-migration rethrow) on a DB failure
- [x] `savedContentRepo.update` resolves to the same `boolean` shape on
      success and rejects with `StorageError` on a DB failure
- [x] `savedContentRepo.remove` resolves to the same `boolean` shape on
      success and rejects with `StorageError` on a DB failure
- [x] `storage.savedContent.list/create/update/remove` each delegate to the
...
- [x] `encounterRepo.loadEncountersByIds` resolves to the same `Encounter[]`
      shape as the pre-migration method on success (scenario: "Method
      resolves through its domain repo")
- [x] `encounterRepo.loadEncountersByIds` logs `outcome: "not_found"` on an
      empty result and returns `[]` immediately (no DB call) when passed an
      empty `ids` array, matching the pre-migration short-circuit
- [x] `encounterRepo.loadEncountersByIds` rejects with `StorageError` on a DB
      failure, asserts exactly one `logStorageEvent` error event
- [x] `encounterRepo.addEncounterToCampaign` resolves to `void` on success
      ...
      write")
- [x] `encounterRepo.removeEncounterFromCampaign` resolves to `void` on
      success and rejects with `StorageError` (collection: `"campaigns"`) on
      a DB failure
- [x] `storage.loadEncountersByIds`/`addEncounterToCampaign`/
...
- [x] `grep -c "console\.\(error\|warn\)" lib/storage.ts` returns `0` after
...
- [x] Manual/scripted re-grep of all 6 confirmed caller files for the 7
...
- [x] Full existing Jest/Vitest suite passes unmodified after Tasks A–C,
      with zero new failures attributable to this change (any pre-existing
      flake is out of scope)
- [x] Type check (`tsc`/`next build` type-checking) passes with no new errors
