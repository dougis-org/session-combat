...
- [ ] 1.1 Confirm a clean starting point by checking out `main`, pulling the latest remote changes, and creating a feature branch for `increase-test-coverage`.
- [ ] 1.2 Record ownership in the implementation notes: implementer, intended reviewer, explicit human approval expectation before merge, and the linkage to GitHub issue `#72`.
- [ ] 1.3 Reproduce the current baseline with the coverage-producing suites and capture the local totals that correspond to Codacy using `npm run test:unit -- --coverageReporters=json-summary --coverageReporters=lcov` and `npm run test:integration -- --coverage --coverageReporters=json-summary --coverageReporters=lcov`.
- [ ] 1.4 Document which CI jobs currently contribute LCOV to Codacy and which suites validate behavior without contributing coverage.
- [ ] 1.5 Audit the current Playwright route coverage and record the already-exercised client surfaces reached through `/register`, `/login`, `/characters`, `/parties`, `/monsters/import`, `/encounters`, and `/combat`.
...
- [ ] 3.1 Write failing tests first for undercovered authenticated API routes and shared server helpers, starting with auth, character, party, encounter, and combat route behavior.
- [ ] 3.2 Implement only the minimum code or test harness changes needed to make the new API and helper tests pass without weakening existing assertions.
- [ ] 3.3 Add focused coverage for `lib/auth.ts`, `lib/db.ts`, `lib/storage.ts`, and adjacent high-centrality helpers where the new tests expose missing seams or edge cases.
- [ ] 3.4 Review the new tests for duplication, brittle fixtures, and unnecessary mocks before moving to the next wave.
...
- [ ] 5.2 Run `npm run lint`, `npm run build`, `npm run test:unit`, and `npm run test:integration`, and record the resulting coverage delta for the milestone.
...
- [ ] 6.1 Prepare a PR that references GitHub issue `#72` and summarizes the verified baseline, the modules covered in this milestone, the measured coverage delta, and any denominator decisions.
