...
- **THEN** `cm-iymrith` and `cm-maegera` are present in `CUSTOM_MONSTERS` with
  full legendary action blocks
...
- **THEN** the demon-lord stat blocks (e.g. `cm-lolth`, `cm-grazzt`,
  `cm-demon-lord-priest`) use canonical `DamageType` values only
