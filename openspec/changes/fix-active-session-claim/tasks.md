...
- [x] **Issue lifecycle: mark in-progress** _(skip if change is not issue-driven)_: run `gh issue edit #564 --add-label "in-progress"`. Then discover the GitHub Project linked to the repo (`gh project list --owner <owner> --format json`), resolve the status field option semantically matching "In Progress" (`gh project field-list <project-number> --owner <owner> --format json`), and move the project item via `gh project item-edit`. If no project item is found, log a warning and continue. If the `gh` token lacks the `project` scope, surface a message instructing the user to run `gh auth refresh -s project` and skip the project-item update (issue label update still proceeds).
- [x] In `app/api/campaigns/[id]/sessions/active/route.ts`:
  - Wrap `await storage.saveSessionLog(log);` in a `try...catch` block.
  - In the catch block, call `await storage.setActiveCampaignSession(campaignId, campaign.userId, null);` to release the active session claim.
  - Log the error (`console.error('Error saving session log:', error);`).
  - Return a 500 response (`NextResponse.json({ error: 'Failed to create session log' }, { status: 500 })`).
...
- [x] Open PR from working branch to `<default-branch>`. **If this change is issue-driven, the PR body MUST include `Closes #564` for each linked issue** (unconditionally, not as an optional conditional).
- [x] **Issue lifecycle: mark in-review** _(skip if change is not issue-driven)_: run `gh issue edit #564 --add-label "in-review" --remove-label "in-progress"`. Then move the project item to the status column semantically matching "In Review" via `gh project item-edit` (same project/field/option discovery as the in-progress lifecycle step above; warn and skip if not found).
...
- [x] Confirm `openspec/changes/archive/YYYY-MM-DD-fix-active-session-claim/` exists and `openspec/changes/fix-active-session-claim/` is gone
- [x] **Create a doc branch** for the archive and spec updates: `git checkout -b doc/archive-YYYY-MM-DD-fix-active-session-claim` then `git push -u origin doc/archive-YYYY-MM-DD-fix-active-session-claim`
- [x] Open a PR from `doc/archive-YYYY-MM-DD-fix-active-session-claim` to `<default-branch>` with title `docs: archive fix-active-session-claim (YYYY-MM-DD)` — **do NOT push directly to `<default-branch>`**
...
- [x] Prune merged local branches: `git fetch --prune` and `git branch -D <feature-branch> doc/archive-YYYY-MM-DD-fix-active-session-claim`

Required cleanup after archive: `git fetch --prune` and `git branch -D <feature-branch> doc/archive-YYYY-MM-DD-fix-active-session-claim`
