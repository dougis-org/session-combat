...
- [ ] **T3 — `app/api/characters/import/route.ts` (pure):** replace `import { storage } from "@/lib/storage"` with `import { loadCharacters, saveCharacter } from "@/lib/storage/characterRepo"`; update the two call sites. Verify no other `storage.*` calls remain before removing the import.
...
- [ ] Run E2E tests (if applicable): character routes are API-only; skip `test:e2e`/`test:regression` unless a Playwright spec directly exercises one of the six touched routes — check `tests/e2e/` for character-route coverage before skipping
