...
- [ ] **Step 2 — Create and publish working branch:** `git checkout -b enhance-dice-roll-modal` then immediately `git push -u origin enhance-dice-roll-modal`
...
- [x] **Issue lifecycle: mark in-progress** _(skip if change is not issue-driven)_: run `gh issue edit #595 --add-label "in-progress"`. Then discover the GitHub Project linked to the repo (`gh project list --owner dougis-org --format json`), resolve the status field option semantically matching "In Progress" (`gh project field-list <project-number> --owner dougis-org --format json`), and move the project item via `gh project item-edit`. If no project item is found, log a warning and continue. If the `gh` token lacks the `project` scope, surface a message instructing the user to run `gh auth refresh -s project` and skip the project-item update (issue label update still proceeds).
- [x] Update `lib/components/dice/DiceRollOverlay.tsx` to increase the font size of the formula label (e.g. from `text-xs` to `text-2xl font-bold`).
- [x] Add visual SVG rendering to `DiceRollOverlay` (or via a new component like `StaticRollResult`) utilizing `lib/components/icons/dice.tsx`.
- [x] Implement conditional rendering in `DiceRollOverlay`: if `built.percentileFaces` exists, render two `DiceD10Icon`s side-by-side (tens and ones); otherwise map `built.breakdown` and render the matching icon for each die, superimposing its value in the center. Ensure styling allows wrapping for large pools.
- [x] Update `lib/components/GlobalDiceFab.tsx` to call `dp.reset()` inside `performRoll` upon a successful roll dispatch.
...
- [ ] Open PR from working branch to `main`. **If this change is issue-driven, the PR body MUST include `Closes #595` for each linked issue** (unconditionally, not as an optional conditional).
- [ ] **Issue lifecycle: mark in-review** _(skip if change is not issue-driven)_: run `gh issue edit #595 --add-label "in-review" --remove-label "in-progress"`. Then move the project item to the status column semantically matching "In Review" via `gh project item-edit` (same project/field/option discovery as the in-progress lifecycle step above; warn and skip if not found).
...
- Implementer: Agent
- Reviewer(s): Codeowner / Review Toolkit
...
- [ ] Open a PR from `doc/archive-YYYY-MM-DD-<name>` to `main` with title `docs: archive <name> (YYYY-MM-DD)` — **do NOT push directly to `main`**
