...
- [ ] 1.1 Create `lib/dice/useDicePoolState.ts`: extract pool/modifier state, add/remove
...
- [ ] 1.2 Unit test `useDicePoolState` in isolation (add/remove clamping, modifier
      clamping, open/close on outside-click/Escape, `buildRoll()` output shape)
- [ ] 1.3 Create `lib/dice/useRollSubmission.ts`: extract `submitRoll(campaignId, formula,
...
- [ ] 1.4 Unit test `useRollSubmission` in isolation (success/conflict/error/thrown-error
      cases, mocking `fetch`)

## 2. Shared dice presentational components (`lib/components/dice/`)

- [ ] 2.1 Create `lib/components/dice/DicePoolPanel.tsx`, moved from `CampaignChat.tsx`,
      driven by a `dp`-shaped prop matching `useDicePoolState`'s return value plus a
      passed-in `onRoll`/submission handler
- [ ] 2.2 Create `lib/components/dice/DiceTriggerButton.tsx`, moved from
...
- [ ] 3.1 In `CampaignChat.tsx` (still the single file at this point), replace the inline
...
- [ ] 3.2 Run the full existing dice-pool test suite
...
- [ ] 4.1 Delete `CampaignChat.tsx`'s `handleRollPosted` function and its direct call from
...
- [ ] 4.2 Delete `CampaignChat.tsx`'s `useEffect` that calls `onRollRequested`, and its
      import of `onRollRequested` from `lib/dice/diceSessionBridge.ts`
- [ ] 4.3 Re-run the repo-wide search for `requestRoll`/`onRollRequested` usages to confirm
...
- [ ] 4.4 Remove `requestRoll`, `onRollRequested`, `RollRequestPayload`,
      ...
      unchanged
- [ ] 4.5 Delete `tests/unit/components/CampaignChat/CampaignChat.diceSessionBridge.test.tsx`
      in its entirety (covers only the removed request/response path)
- [ ] 4.6 Update `tests/unit/lib/dice/diceSessionBridge.test.ts` to drop all
      `requestRoll`/`onRollRequested` coverage, keeping presence-channel coverage intact
- [ ] 4.7 Update `CampaignChat.sse.test.tsx` / relevant test file(s) if any assertion
...
- [ ] 5.1 Replace `GlobalDiceFab.tsx`'s inline pool state (`pool`, `modifierText`,
      `handleAdd`, `handleRemove`, `poolTotal`) with `useDicePoolState`
- [ ] 5.2 Replace `GlobalDiceFab.tsx`'s `requestRoll` call in `handleSendToChat` with a
...
- [ ] 5.3 Preserve `GlobalDiceFab`'s existing two-step interaction (roll locally via
...
- [ ] 5.4 Update `tests/unit/components/GlobalDiceFab.test.tsx`: replace
...
- [ ] 6.1 Create `lib/components/CampaignChat/useDockState.ts`: `dockReducer`,
...
- [ ] 6.2 Create `lib/components/CampaignChat/useChatFeed.ts`: `feed`/`seenIds` state, SSE
...
- [ ] 6.3 Create `lib/components/CampaignChat/ChatFeed.tsx`: `ChatFeed`,
      `ChatMessageItem`, `RollFeedItem`, `getVisibilityMarker`, `resolveUsername`
- [ ] 6.4 Create `lib/components/CampaignChat/Composer.tsx`: `ChatComposer`,
      `MentionDropdown`
- [ ] 6.5 Create `lib/components/CampaignChat/DragHandle.tsx` (trivial move, unchanged)
- [ ] 6.6 Create `lib/components/CampaignChat/index.tsx`: coordinator wiring
...
- [ ] 6.7 Delete the original `lib/components/CampaignChat.tsx`
- [ ] 6.8 Update any relative imports inside the moved test helpers/files if needed (test
...
- [ ] 7.1 Run `npm run test:unit` — full suite, including all 13
...
- [ ] 7.2 Run the project's typecheck/build to confirm no remaining import of the removed
...
- [ ] 7.4 Confirm no file under `lib/components/CampaignChat/` or `lib/dice/` trips the
