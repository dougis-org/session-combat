...
- Constraints:
  - `Open5EClient` has no built-in retry/backoff (see `lib/import/open5eAdapter.ts:134-178`); its `fetchMonsters`/`fetchSpells` throw immediately on a non-ok response. The manual script needs to decide whether to keep retry behavior and, if so, where it lives (script-local vs. added to the client).
