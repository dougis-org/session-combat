...
- [ ] Archive the change: move `openspec/changes/fix-audit-cves/` to `openspec/changes/archive/YYYY-MM-DD-fix-audit-cves/` **and stage both the new location and the deletion of the old location in a single commit** — do not commit the copy and delete separately
- [ ] Confirm `openspec/changes/archive/YYYY-MM-DD-fix-audit-cves/` exists and `openspec/changes/fix-audit-cves/` is gone
- [ ] **Create a doc branch** for the archive and spec updates: `git checkout -b doc/archive-YYYY-MM-DD-fix-audit-cves` then `git push -u origin doc/archive-YYYY-MM-DD-fix-audit-cves`
- [ ] Open a PR from `doc/archive-YYYY-MM-DD-fix-audit-cves` to `main` with title `docs: archive fix-audit-cves (YYYY-MM-DD)` — **do NOT push directly to `main`**
